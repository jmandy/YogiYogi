...
class WaitRoomDisplay extends JFrame implements ActionListener, KeyListener, MouseListener, ChangeListener {
	...
	public WaitRoomDisplay(ClientThread thread) {
		...
		c.add(join);
		/*
		 * sendword = new JButton("귓속말ㅋㅋ"); sendword.setFont(font);
		 * sendword.setBounds(500, 330, 100, 30); sendword.setBorder(new
		 * SoftBevelBorder(SoftBevelBorder.RAISED)); sendword.addActionListener(this);
		 * c.add(sendword);
		 */
		logout = new JButton("로 그 아 웃");
		logout.setFont(font);
		logout.setBounds(500, 330, 100, 30);
		logout.setBorder(new SoftBevelBorder(SoftBevelBorder.RAISED));
		logout.addActionListener(this);
		c.add(logout);

		Dimension dim = getToolkit().getScreenSize();
		setSize(640, 500);// 전체 창 크기 증가.
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == create) {
			...
			if (!isSelected) {
				JOptionPane.showMessageDialog(this, "들어가고 싶은 방 골라요!!", "대화방 입장.", JOptionPane.ERROR_MESSAGE);
